...
describe("describeDbError", () => {
  it("explains a missing table (Postgres 42P01)", () => {
    const msg = describeDbError({ code: "42P01", message: 'relation "employer_company_profiles" does not exist' });
    expect(msg).toMatch(/migrations/i);
  });

  it("explains a PostgREST schema-cache miss", () => {
    const msg = describeDbError({ code: "PGRST205", message: "Could not find the table 'public.employer_job_listings' in the schema cache" });
    expect(msg).toMatch(/migrations/i);
  });

  it("explains an RLS denial", () => {
    const msg = describeDbError({ code: "42501", message: "new row violates row-level security policy" });
