...
export function useAuthMFA() {
    ...
    async (code: string): Promise<boolean> => {
      ...
      if (verifyError) {
        setEnrollState({
          status: "enrolling",
          factorId: enrollState.factorId,
          qrCode: (enrollState as Extract<MFAEnrollState, { status: "verifying" | "enrolling" } & { qrCode: string }>).qrCode ?? "",
  ...
  const verifyChallenge = useCallback(
    async (factorId: string, code: string): Promise<boolean> => {
      ...
      if (challengeError || !challengeData) {
        setChallengeState({ status: "error", message: challengeError?.message ?? "Challenge failed" });
        return false;
      }
