...
function SkillPill({ skill, onRemove }: { skill: string; onRemove: () => void }) {
  return (
    ...
    >
      {skill}
      <button onClick={onRemove} className="opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "var(--primary)" }}>
        <X className="w-3 h-3" />
      </button>
    </span>
  );
...
function TimelineCard({
  icon, title, subtitle, meta, description,
  onEdit, onDelete, editing, fields, onCommit, onCancel,
}: {
  icon: React.ReactNode; title: string; subtitle: string; meta: string;
  description?: string; onEdit: () => void; onDelete: () => void;
  editing: boolean; fields: React.ReactNode; onCommit: () => void; onCancel: () => void;
}) {
  if (editing) {
    return (
      <div className="rounded-2xl p-4" style={{ background: "var(--card)", border: "1.5px solid var(--primary)" }}>
        ...
        <div className="flex gap-2 mt-3">
          <button onClick={onCommit} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold" style={{ background: "var(--primary)", color: "#fff" }}>
            <Check className="w-3.5 h-3.5" /> Save
          </button>
          <button onClick={onCancel} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium" style={{ border: "1px solid var(--border)", color: "var(--muted-foreground)" }}>
  ...
  return (
    <div className="flex gap-4 p-4 rounded-2xl group transition-all hover:shadow-sm" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: "var(--muted)" }}>
        {icon}
      </div>
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <div className="text-xs mt-0.5" style={{ color: "var(--muted-foreground)" }}>
              {subtitle}{meta && <> · <span>{meta}</span></>}
            </div>
          </div>
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
            <button onClick={onEdit} className="p-1.5 rounded-lg" style={{ background: "var(--muted)" }}>
              <Pencil className="w-3 h-3" style={{ color: "var(--muted-foreground)" }} />
            </button>
            <button onClick={onDelete} className="p-1.5 rounded-lg" style={{ background: "var(--muted)" }}>
              <Trash2 className="w-3 h-3" style={{ color: "var(--muted-foreground)" }} />
            </button>
          </div>
        </div>
        {description && (
          <p className="text-xs mt-2 leading-relaxed line-clamp-2" style={{ color: "var(--muted-foreground)" }}>{description}</p>
        )}
      </div>
    </div>
  );
...
const iStyle: React.CSSProperties = {
  ...
  height: 38, fontSize: 13, padding: "0 12px", color: "var(--foreground)",
  width: "100%", outline: "none", fontFamily: "inherit",
};
const lStyle: React.CSSProperties = {
  fontSize: 11, fontWeight: 600, color: "var(--muted-foreground)", marginBottom: 4,
...
export function ProfileSettings() {
  ...
  const [newWork, setNewWork] = useState<WorkExperience>({ id: "", company: "", role: "", startDate: "", endDate: "", responsibilities: "" });

  /* education state */
  const [education, setEducation] = useState<Education[]>(profile.education?.length ? profile.education : []);
  ...
  function commitWork() {
    ...
    setEditingWorkId(null); setWorkDraft(null);
  }
  ...
  function commitEdu() {
    ...
    setEditingEduId(null); setEduDraft(null);
  }
    ...
    return (
      <div className="grid grid-cols-2 gap-2">
        ...
        <div><label style={lStyle}>Company</label>
          <ComboInput style={iStyle} value={data.company} onChange={(v) => onChange("company", v)} options={SP500_COMPANIES} placeholder="Acme Corp" /></div>
        ...
        <div className="col-span-2"><label style={lStyle}>Highlights</label>
          <textarea value={data.responsibilities} placeholder="Key achievements and responsibilities…" rows={3}
            ...
            style={{ ...iStyle, height: "auto", padding: "8px 12px", resize: "none" }} /></div>
      </div>
    );
  }
      ...
      <div className="relative flex-shrink-0">
        ...
        <div className="absolute left-8" style={{ bottom: -44 }}>
          <div
            ...
            style={{ background: "var(--primary)", color: "#fff", borderColor: "var(--background)", fontFamily: "var(--font-display)" }}
          >
            {initials}
          </div>
        </div>
        <div className="absolute top-4 right-6">
          <button
            ...
            style={{ background: saved ? "var(--forest)" : "rgba(255,255,255,0.92)", color: saved ? "#fff" : "var(--foreground)", backdropFilter: "blur(8px)" }}
          >
      ...
      <div className="px-8 pt-14 pb-5 flex-shrink-0" style={{ background: "var(--card)", borderBottom: "1px solid var(--border)" }}>
        <div className="flex-1 min-w-0">
          ...
          <input
            ...
            style={{ color: "var(--primary)", fontWeight: 600, border: "none", borderBottom: "2px solid transparent", paddingBottom: 2, transition: "border-color 0.15s" }}
        ...
        <textarea
          ...
          rows={3} className="w-full text-sm bg-transparent outline-none resize-none mt-4"
          style={{ color: summary ? "var(--foreground)" : "var(--muted-foreground)", fontFamily: "inherit", lineHeight: 1.7, border: "none", borderBottom: "2px solid transparent", transition: "border-color 0.15s" }}
        ...
        <div className="flex flex-wrap gap-3 mt-4">
          {[
            ...
            <div key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs" style={{ background: "var(--muted)", border: "1px solid var(--border)" }}>
        ...
        <div className="col-span-1 p-6 flex flex-col gap-6" style={{ borderRight: "1px solid var(--border)" }}>
          <section>
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {skillsList.map((s) => (
                <SkillPill key={s} skill={s} onRemove={() => setSkillsList((p) => p.filter((x) => x !== s))} />
              ))}
              {skillsList.length === 0 && (
                <p className="text-xs italic" style={{ color: "var(--muted-foreground)" }}>No skills yet — browse above or type below.</p>
              )}
            </div>
            <div className="flex gap-2">
              <input type="text" value={newSkill} onChange={(e) => setNewSkill(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && addSkill()} placeholder="Add a skill…"
        ...
        <div className="col-span-2 p-6 flex flex-col gap-8 overflow-y-auto">
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xs font-bold tracking-widest uppercase" style={{ color: "var(--muted-foreground)" }}>Experience</h3>
              <button
            ...
            <div className="flex flex-col gap-3">
              ...
              {workHistory.length === 0 && !addingWork && (
                <p className="text-xs italic py-2" style={{ color: "var(--muted-foreground)" }}>No experience added yet.</p>
              )}
              {workHistory.map((w) => (
                <TimelineCard key={w.id}
                  ...
                  editing={editingWorkId === w.id} onCommit={commitWork}
                  onCancel={() => { setEditingWorkId(null); setWorkDraft(null); }}
          ...
          <section className="pb-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              {education.length === 0 && !addingEdu && (
                <p className="text-xs italic py-2" style={{ color: "var(--muted-foreground)" }}>No education added yet.</p>
              )}
              {education.map((e) => (
                <TimelineCard key={e.id}
                  icon={<GraduationCap className="w-4 h-4" style={{ color: "var(--forest)" }} />}
                  title={e.degree || e.university} subtitle={e.university} meta={e.graduationYear}
                  onEdit={() => { setEditingEduId(e.id); setEduDraft({ ...e }); }}
                  onDelete={() => setEducation((p) => p.filter((x) => x.id !== e.id))}
                  editing={editingEduId === e.id} onCommit={commitEdu}
                  onCancel={() => { setEditingEduId(null); setEduDraft(null); }}
