...
export function CompanyBrowser({ onPick }: Props) {
  ...
  const catalog = useMemo<CatalogEntry[]>(() => {
    ...
    for (const r of db) {
      ...
      if (existing)
        Object.assign(existing, { ready: true, logoUrl: r.logoUrl, industry: r.industry });
      else
        bySlug.set(r.slug, {
          slug: r.slug,
          name: r.name,
      ...
      <MentorCard style={{ overflow: "hidden" }}>
        <div style={{ padding: "18px 20px" }}>
          <div
            ...
            style={{ fontSize: 18, fontWeight: 600, color: "var(--foreground)", marginBottom: 4 }}
          >
            Research a company
          </div>
          <div style={{ fontSize: 13, color: "var(--muted-foreground)", marginBottom: 14 }}>
            Pick one from the library below
            {readyCount ? ` (${readyCount} with verified profiles)` : ""}, or search for any
            company.
          </div>
          ...
          {q && !exactMatch && (
            ...
            >
              <Sparkles className="w-4 h-4" /> Research “{query.trim()}”{" "}
        ...
        !q && (
          <MentorCard style={{ padding: 32, textAlign: "center" }}>
            <div style={{ fontSize: 14, color: "var(--muted-foreground)" }}>
              No companies in the library yet — search above to research one.
            </div>
          </MentorCard>
        )
      )}
