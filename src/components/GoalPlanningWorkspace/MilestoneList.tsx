...
import { Loader2, Trash2, Target, ChevronDown, ChevronRight } from "lucide-react";
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: "14px 18px",
        background: "var(--card)",
        border: `1px solid ${editingPlanId === p.id ? "var(--primary)" : "var(--border)"}`,
        borderRadius: 14,
      }}
    >
        ...
        {(p.milestones?.length ?? 0) > 0 && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6 }}>
            ...
            >
              <div
                style={{
                  width: `${Math.round((p.milestones!.filter((m) => m.done).length / p.milestones!.length) * 100)}%`,
                  height: "100%",
      ...
      <button
        ...
        style={{
          ...
          fontWeight: 600,
          color: "#fff",
          cursor: loadingPlanId === p.id ? "not-allowed" : "pointer",
          opacity: loadingPlanId === p.id ? 0.7 : 1,
          display: "flex",
          alignItems: "center",
          gap: 6,
        }}
            ...
            <div key={type}>
              ...
              {!collapsed && (
                <div className="flex flex-col gap-3">
                  {plans.map((p) => (
                    <PlanCard key={p.id} p={p} {...cardProps} />
                  ))}
                </div>
              )}
            </div>
