      ...
      <header style={{ padding: "16px 24px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 16, flexShrink: 0 }}>
        ...
        {savedPlans.length > 1 && (
          <div style={{ position: "relative" }}>
            ...
            {switcherOpen && (
              <>
                <div onClick={onSwitcherClose} style={{ position: "fixed", inset: 0, zIndex: 40 }} />
                  ...
                  {savedPlans.slice().sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((p) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => { onSwitcherClose(); if (!isCurrent) onOpenPlan(p); }}
      ...
      <div className="flex-1 flex overflow-hidden">
              ...
              {!isGenerating && planMarkdown.trim() && (
                editingSheet ? (
                  <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                    <button onClick={onCancelEdit} style={{ background: "transparent", border: "1px solid var(--border)", borderRadius: 8, height: 30, padding: "0 12px", cursor: "pointer", color: "var(--muted-foreground)", fontFamily: "inherit", fontSize: 12, fontWeight: 600 }}>Cancel</button>
                    <button onClick={onApplyEdit} style={{ display: "flex", alignItems: "center", gap: 5, background: "var(--primary)", border: "none", borderRadius: 8, height: 30, padding: "0 12px", cursor: "pointer", color: "#fff", fontFamily: "inherit", fontSize: 12, fontWeight: 600 }}><Check className="w-3.5 h-3.5" /> Done</button>
                  </div>
                ) : (
                  <button onClick={onStartEdit} style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 5, background: "transparent", border: "1px solid var(--border)", borderRadius: 8, height: 30, padding: "0 12px", cursor: "pointer", color: "var(--foreground)", fontFamily: "inherit", fontSize: 12, fontWeight: 600 }}><Pencil className="w-3.5 h-3.5" /> Edit</button>
                )
              )}
            </div>
        ...
        <div className="w-[400px] h-full border-l border-border flex flex-col" style={{ background: "var(--card)" }}>
          ...
          <div className="border-t border-border" style={{ padding: 12, background: "var(--muted)" }}>
            ...
            <form onSubmit={onSend} className="flex items-center gap-2 rounded-[14px] border border-border" style={{ background: "var(--card)", padding: "8px 8px 8px 14px" }}>
