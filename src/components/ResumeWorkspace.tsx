...
import { Loader2, Download, Save, Edit3, Eye, CheckCircle2, Sparkles, Send, X } from "lucide-react";
...
const QUICK_ACTIONS = [
  {
    label: "Strengthen",
    instruction:
      "Rewrite using a stronger action verb and the XYZ formula (Action + Metric + Result).",
  },
  {
    label: "Add metrics",
    instruction: "Add specific quantified metrics or percentages to make this more data-driven.",
  },
  { label: "Fix grammar", instruction: "Fix any grammar, tense, or punctuation issues." },
  {
    label: "Simplify",
    instruction: "Simplify and tighten this — remove filler words and make it more concise.",
  },
];
...
export function ResumeWorkspace({
  ...
  const handleRewrite = useCallback(async () => {
    ...
    setIsRewriting(true);
    try {
      const rewritten = await rewriteResumeSelection(
  ...
  return (
    <div
      className="flex flex-col h-full w-full absolute inset-0 z-50 overflow-hidden"
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ background: "rgba(217,119,87,0.10)", color: "var(--primary)" }}
          >
          ...
          <div>
            ...
            <div className="text-xs" style={{ color: "var(--muted-foreground)" }}>
              ...
              {saveStatus === "saved" && (
                <span className="flex items-center gap-1" style={{ color: "var(--forest)" }}>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0">
        {/* Left mini toolbar */}
        <div
          className="w-full md:w-16 flex flex-row md:flex-col items-center justify-center md:justify-start gap-8 md:gap-6 py-3 md:py-6 shrink-0 print:hidden"
            ...
            >
              <div
                style={{
                  padding: 12,
                  borderRadius: 12,
                  background: activeTab === id ? "rgba(217,119,87,0.20)" : "transparent",
                  color: activeTab === id ? "var(--primary)" : "rgba(251,247,241,0.45)",
                  transition: "all 0.2s",
                }}
              >
              ...
              <span
                style={{
                  fontSize: 10,
                  fontWeight: 600,
                  color: activeTab === id ? "var(--primary)" : "rgba(251,247,241,0.4)",
        ...
        {/* Resume canvas */}
        <div
          className="flex-1 min-h-0 overflow-y-auto no-scrollbar flex justify-center items-start py-4 px-2 sm:py-10 sm:px-8 print:p-0"
          style={{ background: "var(--muted)" }}
        >
          <div
            ref={canvasRef}
        ...
        {/* Right panel */}
        <div
          className="w-full md:w-[400px] h-[55vh] md:h-auto flex flex-col shrink-0 border-t md:border-t-0 print:hidden"
        ...
        <div
          ...
          style={{
            ...
            left: Math.max(
              8,
              Math.min(
                selectionToolbar.left + selectionToolbar.width / 2 - 180,
            ...
            {QUICK_ACTIONS.map((action) => (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  background:
                    selectionInstruction === action.instruction
                      ? "var(--primary)"
                      : "rgba(251,247,241,0.10)",
                  color:
                    selectionInstruction === action.instruction ? "#fff" : "rgba(251,247,241,0.75)",
                  border:
                    selectionInstruction === action.instruction
                      ? "1px solid var(--primary)"
                      : "1px solid rgba(251,247,241,0.15)",
                  transition: "all 0.15s",
                }}
              >
                {action.label}
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input
              ...
              style={{
                ...
                padding: "0 12px",
                borderRadius: 8,
                border: "1px solid rgba(251,247,241,0.20)",
                background: "rgba(251,247,241,0.08)",
                color: "rgba(251,247,241,0.90)",
                fontSize: 12,
                fontFamily: "inherit",
                outline: "none",
              }}
            ...
            <button
              ...
              style={{
                ...
                background:
                  selectionInstruction.trim() && !isRewriting
                    ? "var(--primary)"
                    : "rgba(251,247,241,0.10)",
                color: "#fff",
                display: "flex",
