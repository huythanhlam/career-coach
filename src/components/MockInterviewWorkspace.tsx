...
import {
  ...
  CheckCircle2, Lightbulb, Play, Mic, MicOff, Volume2, VolumeX, Clock, AlertTriangle,
  RotateCcw, Bot, MessageSquare, PhoneOff, Captions, X, Radio, Check,
} from "lucide-react";
...
import {
  KOKORO_VOICES, DEFAULT_KOKORO_VOICE, preloadKokoro, kokoroGenerateSamples, isKokoroVoice, type KokoroVoice,
} from "@/services/kokoroTts";
import { playKokoroSample, stopKokoroSample, resumeAudioContext } from "@/services/kokoroAudio";
import { QUESTION_BANK } from "@/config/interviewQuestions";
import {
  SCORE_DIMENSIONS, MOCK_WORKFLOW_LABELS, STAR_ELEMENTS,
  type MockWorkflowId, type InterviewSession, type ChatTurn, type QuestionFeedback,
      ...
      {custom ? (
        <div>
          ...
          <button
            ...
            style={{ display: "inline-flex", alignItems: "center", gap: 4, marginTop: 6, background: "transparent", border: "none", padding: 0, cursor: "pointer", color: "var(--primary)", fontFamily: "inherit", fontSize: 12, fontWeight: 600 }}
          >
...
function QuestionFeedbackCard({ qf, index }: { qf: QuestionFeedback; index: number }) {
  const qualityColor = qf.quality === "Strong" ? "var(--forest)" : qf.quality === "Adequate" ? "var(--marigold, #E8B948)" : "var(--primary)";
  return (
    <div style={{ ...cardStyle, padding: 20 }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--muted-foreground)", marginBottom: 4 }}>
            Question {index + 1}
          </div>
          <div style={{ fontSize: 14, fontWeight: 600, color: "var(--foreground)", lineHeight: 1.45 }}>{qf.question}</div>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <div className="font-display" style={{ fontSize: 24, fontWeight: 700, lineHeight: 1, color: qualityColor }}>{qf.score}</div>
          <div style={{ marginTop: 4, fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.06em", color: qualityColor, background: "color-mix(in srgb, " + qualityColor + " 12%, transparent)", borderRadius: 6, padding: "2px 7px" }}>
            {qf.quality}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 12 }}>
        {STAR_ELEMENTS.map(({ key, label }) => {
          ...
          const present = !!value;
          return (
            <div key={key} style={{ display: "flex", gap: 8, alignItems: "baseline", fontSize: 12.5, lineHeight: 1.5 }}>
              <span style={{ width: 74, flexShrink: 0, fontWeight: 700, color: present ? "var(--foreground)" : "var(--primary)" }}>{label}</span>
              {present ? (
                <span style={{ color: "var(--muted-foreground)" }}>{value}</span>
              ) : (
                <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontWeight: 600, color: "var(--primary)" }}>
      ...
      {qf.feedback && (
        <p style={{ fontSize: 13, color: "var(--muted-foreground)", lineHeight: 1.6, margin: 0, paddingTop: 12, borderTop: "1px solid var(--border)" }}>
          {qf.feedback}
        </p>
      )}
    </div>
...
function CallTile({
  name, sublabel, initials, active, accent, muted,
}: {
  name: string;
  sublabel?: string;
  initials?: string;
  ...
  return (
    <div style={{
      position: "relative", flex: "1 1 320px", maxWidth: 560, minHeight: 220,
      aspectRatio: "16 / 10", borderRadius: 18, background: "#1b1e25",
      ...
      <div style={{
        width: 104, height: 104, borderRadius: "50%", background: `${accent}22`,
        color: accent, border: `2px solid ${accent}66`, display: "flex",
        alignItems: "center", justifyContent: "center", fontSize: 34, fontWeight: 700,
      }}>
      ...
      {active && (
        <div aria-hidden style={{ position: "absolute", top: 14, right: 16, display: "flex", gap: 3, alignItems: "flex-end", height: 22 }}>
          {[0, 1, 2, 3, 4].map((i) => (
            <span key={i} className="mi-eqbar" style={{ width: 3, height: 5, background: accent, borderRadius: 2, animationDelay: `${i * 0.11}s` }} />
          ))}
        </div>
      )}

      <div style={{ position: "absolute", bottom: 12, left: 12, display: "flex", alignItems: "center", gap: 6, background: "rgba(0,0,0,0.5)", color: "#fff", padding: "4px 10px", borderRadius: 8, fontSize: 12.5, fontWeight: 600 }}>
        {muted ? <MicOff className="w-3.5 h-3.5" style={{ opacity: 0.85 }} /> : <Mic className="w-3.5 h-3.5" style={{ opacity: 0.85 }} />}
        {name}{sublabel ? <span style={{ opacity: 0.7, fontWeight: 500 }}>· {sublabel}</span> : null}
      </div>
    </div>
  );
}
...
function CallControl({
  onClick, active, disabled, title, icon, text,
}: {
  onClick: () => void;
  active?: boolean;
  disabled?: boolean;
  ...
  return (
    <button onClick={onClick} disabled={disabled} aria-label={title} aria-pressed={active} title={title}
      style={{
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3,
        width: 66, height: 56, borderRadius: 12, border: "none",
        cursor: disabled ? "not-allowed" : "pointer",
        background: active ? "rgba(217,119,87,0.22)" : "#23262e",
        color: active ? "#ffb59b" : "#cdd2da", fontFamily: "inherit", fontSize: 10, fontWeight: 600,
...
export function MockInterviewWorkspace({ workflowId }: Props) {
  ...
  // Voice previews play through the shared Web Audio context; stop any in-flight
  // preview on unmount so it doesn't outlive the screen.
  useEffect(() => stopKokoroSample, []);
  ...
  const dictation = useDictation({
    onText: (t) => {
      ...
      silenceTimerRef.current = setTimeout(() => {
        ...
        try { dictation.stop(); } catch { /* ignore */ }
  ...
  useEffect(() => {
    ...
    if (secondsLeft <= 0) { setSecondsLeft(null); setTimeUp(true); return; }
  ...
  const handleStart = async () => {
    ...
    if (voiceEnabled) { preloadKokoro(); resumeAudioContext(); } // warm + unlock audio on this gesture
  ...
  const handleSend = async (e?: React.FormEvent, overrideText?: string) => {
    ...
    clearSilenceTimer();
    hadSpeechRef.current = false;
    if (dictation.listening) { try { dictation.stop(); } catch { /* ignore */ } }
    setInput("");
  ...
  useEffect(() => { handleSendRef.current = (t: string) => handleSend(undefined, t); });
  ...
  const toggleVoice = () => {
    setVoiceOn((on) => {
      ...
      else resumeAudioContext(); // turning on — unlock audio on this gesture
      return !on;
    });
  };
  ...
  const sampleVoice = async (voiceId: string) => {
    ...
    speech.cancel();
    stopKokoroSample();
    resumeAudioContext(); // this tap is the user gesture that unlocks audio
    setVoiceError(null);
    setSamplingVoice(voiceId);
    try {
      const { samples, sampleRate } = await kokoroGenerateSamples(
        "Hi, I'm your interviewer today. Let's get started — tell me about yourself.",
        voiceId,
        { waitForLoad: true },
      );
      await playKokoroSample(samples, sampleRate);
    } catch (err) {
    ...
    return (
      <div className="flex-1 flex flex-col h-full overflow-hidden" style={{ background: "var(--background)" }}>
        <header style={{ padding: "16px 24px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 16, flexShrink: 0 }}>
          <button onClick={reset} aria-label="Done — back to setup"
          ...
          <button onClick={reset} aria-label="Practice again"
            ...
            {s.questionFeedback?.length ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 600, color: "var(--foreground)" }}>Per-question feedback</span>
                  <span style={{ fontSize: 12, color: "var(--muted-foreground)" }}>STAR breakdown of each answer</span>
                </div>
                {s.questionFeedback.map((qf, i) => <QuestionFeedbackCard key={i} qf={qf} index={i} />)}
              </div>
            ) : null}
  ...
  if (mode.kind === "live") {
    ...
    const statusText = deriveChatStatus({
      isScoring, isGenerating, speaking: speech.speaking, listening: dictation.listening,
    });
    const initials = (profile.preferredName || profile.fullName || "You")
    ...
    return (
      <div className="flex-1 flex flex-col h-full overflow-hidden relative" style={{ background: "#0f1115" }}>
        <style>{`@keyframes mi-eq{0%,100%{height:5px}50%{height:20px}}.mi-eqbar{animation:mi-eq .8s ease-in-out infinite}`}</style>

        {/* Meeting top bar */}
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px", background: "#16181d", borderBottom: "1px solid #23262e", flexShrink: 0 }}>
          <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 7, color: "#fff", fontWeight: 600, fontSize: 14 }}>
              <span style={{ width: 8, height: 8, borderRadius: 9999, background: "#ef4444" }} /> {config.title}
            </span>
            <span title="You're talking to an AI interviewer" style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 700, color: "#fff", background: "rgba(217,119,87,0.28)", borderRadius: 6, padding: "2px 7px" }}>
              <Bot className="w-3 h-3" /> AI
            </span>
            <span style={{ fontSize: 12, color: "#9aa0aa", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {[roleOf(formData), focusOf(formData)].filter(Boolean).join(" · ")}
            </span>
          </div>
          {askedCount > 0 && (
            <span title="Questions asked so far" style={{ fontSize: 12, fontWeight: 700, color: "#cdd2da", background: "#23262e", borderRadius: 8, padding: "6px 10px" }}>Q{askedCount}</span>
          )}
          {secondsLeft != null && (
            <span title="Time remaining — ends automatically at zero" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontVariantNumeric: "tabular-nums", fontSize: 13, fontWeight: 700, color: secondsLeft <= 60 ? "#ff8f6b" : "#cdd2da", background: "#23262e", borderRadius: 8, padding: "6px 10px" }}>
              <Clock className="w-3.5 h-3.5" /> {String(Math.floor(secondsLeft / 60)).padStart(2, "0")}:{String(secondsLeft % 60).padStart(2, "0")}
            </span>
          )}
        </div>
        ...
        <div className="flex-1 flex min-h-0">
          <div style={{ flex: 1, position: "relative", display: "flex", alignItems: "center", justifyContent: "center", gap: 16, padding: 16, flexWrap: "wrap" }}>
            <CallTile name="AI Interviewer" sublabel="Interviewer" active={speech.speaking} accent="#d97757" />
            <CallTile name="You" initials={initials} active={dictation.listening} accent="#3b82f6" muted={!dictation.listening} />

            {/* Live captions */}
            {showCaptions && (captionText || (dictation.listening && input.trim())) && (
              <div style={{ position: "absolute", left: 16, right: 16, bottom: 16, display: "flex", flexDirection: "column", gap: 8, alignItems: "center", pointerEvents: "none" }}>
                {captionText && (
                  <div style={{ maxWidth: 760, background: "rgba(0,0,0,0.66)", color: "#fff", padding: "10px 16px", borderRadius: 12, fontSize: 15, lineHeight: 1.5, textAlign: "center", maxHeight: 120, overflow: "auto" }}>
                    <span style={{ color: "#ffb59b", fontWeight: 700 }}>Interviewer: </span>{captionText}
                  </div>
                )}
                {dictation.listening && input.trim() && (
                  <div style={{ maxWidth: 760, background: "rgba(59,130,246,0.85)", color: "#fff", padding: "8px 14px", borderRadius: 12, fontSize: 14, textAlign: "center" }}>
                    <span style={{ fontWeight: 700 }}>You: </span>{input}
                  </div>
                )}
              </div>
            )}
          ...
          {showChat && (
            <div className="absolute md:static inset-y-0 right-0 z-30 w-full md:w-[360px] flex flex-col" style={{ background: "var(--card)", borderLeft: "1px solid var(--border)" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid var(--border)" }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "var(--foreground)" }}>Chat</span>
                <button onClick={() => setShowChat(false)} aria-label="Close chat" style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--muted-foreground)" }}><X className="w-4 h-4" /></button>
              </div>
              <div className="flex-1 overflow-auto no-scrollbar" style={{ padding: 16 }}
                role="log" aria-live="polite" aria-relevant="additions text" aria-label="Interview conversation">
                <div className="space-y-4">
                  {messages.map((msg, idx) => <ChatBubble key={idx} msg={msg} />)}
                  {failedInput && !isGenerating && (
                    <div className="flex justify-center">
                      <button onClick={retry} aria-label="Retry the last message"
              ...
              <div className="border-t border-border" style={{ padding: 12, background: "var(--muted)" }}>
                {config.suggestedPrompts && config.suggestedPrompts.length > 0 && (
                  <div className="flex gap-2 mb-2 overflow-x-auto no-scrollbar pb-1">
                    {config.suggestedPrompts.slice(0, 3).map((p, i) => (
                      <button key={i} type="button" onClick={() => handleSend(undefined, p)} disabled={isGenerating}
                ...
                <form onSubmit={handleSend} className="flex items-end gap-2 rounded-[14px] border border-border" style={{ background: "var(--card)", padding: 8 }}>
                  <textarea
                    value={input}
                  ...
                  />
                  <button type="submit" disabled={isGenerating || !input.trim()} aria-label="Send answer"
        ...
        {/* Status line */}
        <div aria-live="polite" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 12, fontWeight: 600, color: "#9aa0aa", padding: "6px 0", background: "#16181d" }}>
          <span style={{ width: 6, height: 6, borderRadius: 9999, background: isGenerating || isScoring ? "#E8B948" : speech.speaking ? "#d97757" : dictation.listening ? "#22c55e" : "#3a3f49" }} />
          {statusText}
        </div>

        {/* Control bar (Zoom/Teams-style) */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "14px 16px", background: "#16181d", borderTop: "1px solid #23262e", flexShrink: 0, flexWrap: "wrap" }}>
          {dictation.supported && (
            <CallControl onClick={toggleMic} active={dictation.listening} disabled={isGenerating}
              title={dictation.listening ? "Mute mic and send your answer" : "Unmute mic to answer by voice"}
              icon={dictation.listening ? <Mic className="w-5 h-5" /> : <MicOff className="w-5 h-5" />}
              text={dictation.listening ? "Mic on" : "Mic"} />
          )}
          {dictation.supported && (
            <CallControl
              onClick={() => setConversational((c) => {
                const nowOn = !c;
                if (!nowOn) { clearSilenceTimer(); if (dictation.listening) { try { dictation.stop(); } catch { /* ignore */ } } }
                return nowOn;
              })}
              active={conversational}
              title={conversational ? "Hands-free is on — answers auto-submit when you pause" : "Turn on hands-free conversation"}
              icon={<Radio className="w-5 h-5" />} text="Hands-free" />
          )}
          {speech.supported && (
            <CallControl onClick={toggleVoice} active={voiceOn}
              title={voiceOn ? "Mute interviewer voice" : "Unmute interviewer voice"}
              icon={voiceOn ? <Volume2 className="w-5 h-5" /> : <VolumeX className="w-5 h-5" />} text="Sound" />
          )}
          <CallControl onClick={() => setShowCaptions((c) => !c)} active={showCaptions}
            title={showCaptions ? "Hide captions" : "Show captions"}
            icon={<Captions className="w-5 h-5" />} text="Captions" />
          <CallControl onClick={() => setShowChat((c) => !c)} active={showChat}
            title={showChat ? "Hide chat" : "Show chat"}
            icon={<MessageSquare className="w-5 h-5" />} text="Chat" />
          <button onClick={handleFinish} disabled={isScoring || isGenerating || !answered}
            ...
            style={{ display: "flex", alignItems: "center", gap: 6, height: 56, padding: "0 18px", borderRadius: 12, border: "none", background: "var(--primary)", color: "#fff", fontFamily: "inherit", fontSize: 13, fontWeight: 700, cursor: isScoring || isGenerating || !answered ? "not-allowed" : "pointer", opacity: isScoring || isGenerating || !answered ? 0.6 : 1 }}>
            {isScoring ? <><Loader2 className="w-4 h-4 animate-spin" /> Scoring…</> : <><Flag className="w-4 h-4" /> End &amp; feedback</>}
          </button>
          <button onClick={() => setShowStopConfirm(true)} disabled={isScoring}
            aria-label="Leave interview" title="Leave — this won't be scored or saved"
            style={{ display: "flex", alignItems: "center", gap: 6, height: 56, padding: "0 18px", borderRadius: 12, border: "none", background: "#ef4444", color: "#fff", fontFamily: "inherit", fontSize: 13, fontWeight: 700, cursor: isScoring ? "not-allowed" : "pointer", opacity: isScoring ? 0.6 : 1 }}>
        ...
        {showStopConfirm && (
          <div
            ...
            style={{ position: "absolute", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}
          >
            <div onClick={(e) => e.stopPropagation()} style={{ ...cardStyle, padding: 24, maxWidth: 420, width: "100%" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(217,119,87,0.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <AlertTriangle className="w-4 h-4" style={{ color: "var(--primary)" }} />
                </div>
                <div className="font-display" style={{ fontSize: 17, fontWeight: 600, color: "var(--foreground)" }}>Stop the interview?</div>
              </div>
              <p style={{ fontSize: 13.5, color: "var(--muted-foreground)", lineHeight: 1.6, margin: "0 0 20px" }}>
                Stopping ends this interview now — it <strong style={{ color: "var(--foreground)" }}>won't be scored or saved</strong>, and you'll start over from the beginning. To get feedback instead, use <strong style={{ color: "var(--foreground)" }}>End &amp; get feedback</strong>.
              </p>
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                <button onClick={() => setShowStopConfirm(false)}
                  style={{ height: 40, padding: "0 16px", background: "var(--muted)", border: "1px solid var(--border)", borderRadius: 10, cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 600, color: "var(--foreground)" }}>
                  Keep going
                </button>
                <button onClick={reset}
                  style={{ height: 40, padding: "0 16px", background: "var(--primary)", border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 600, color: "#fff" }}>
                  Stop &amp; start over
                </button>
              </div>
            </div>
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <div style={{ ...cardStyle, padding: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              <Sparkles className="w-4 h-4" style={{ color: "var(--primary)" }} />
              <span style={{ fontSize: 14, fontWeight: 600, color: "var(--foreground)" }}>Questions</span>
            </div>
            <div style={{ display: "flex", gap: 10, marginBottom: questionMode === "pick" ? 16 : 0 }}>
              {([
                { id: "random", title: "Surprise me", sub: "Recommended · role-tailored, random" },
                { id: "pick", title: "Choose questions", sub: "Practice specific ones" },
              ] as const).map((opt) => {
                const on = questionMode === opt.id;
                return (
                  <button key={opt.id} type="button" onClick={() => setQuestionMode(opt.id)}
                    style={{ flex: 1, textAlign: "left", padding: "12px 14px", borderRadius: 12, cursor: "pointer",
                      border: `1px solid ${on ? "var(--primary)" : "var(--border)"}`,
                      background: on ? "rgba(217,119,87,0.08)" : "var(--card)", fontFamily: "inherit" }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: on ? "var(--primary)" : "var(--foreground)" }}>{opt.title}</div>
                    <div style={{ fontSize: 11.5, color: "var(--muted-foreground)", marginTop: 2 }}>{opt.sub}</div>
                  </button>
                );
              })}
            </div>
            ...
            {questionMode === "pick" && (
              <div>
                <div style={{ fontSize: 12, color: "var(--muted-foreground)", marginBottom: 12 }}>
                  Pick the questions you want to practice — the interviewer asks exactly these (rephrased for your role).
                  <strong style={{ color: "var(--foreground)" }}>{" "}{pickedQuestions.length} selected.</strong>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 16, maxHeight: 360, overflow: "auto" }}>
                  {QUESTION_BANK.map((cat) => (
                    <div key={cat.category}>
                      <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--muted-foreground)", marginBottom: 8 }}>{cat.category}</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {cat.questions.map((q) => {
                          const on = pickedQuestions.includes(q);
                          return (
                            <button key={q} type="button" onClick={() => toggleQuestion(q)}
                              style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "9px 12px", borderRadius: 10, cursor: "pointer", textAlign: "left",
                                border: `1px solid ${on ? "var(--primary)" : "var(--border)"}`,
                                background: on ? "rgba(217,119,87,0.08)" : "var(--card)", fontFamily: "inherit" }}>
                              <span style={{ width: 18, height: 18, flexShrink: 0, marginTop: 1, borderRadius: 5, display: "flex", alignItems: "center", justifyContent: "center", border: `2px solid ${on ? "var(--primary)" : "var(--border)"}`, background: on ? "var(--primary)" : "transparent", color: "#fff" }}>
                                {on && <Check className="w-3 h-3" />}
                              </span>
                              <span style={{ fontSize: 13, color: "var(--foreground)", lineHeight: 1.4 }}>{q}</span>
                            </button>
                          );
                        })}
                      </div>
              ...
              {KOKORO_VOICES.map((v: KokoroVoice) => {
                ...
                return (
                  <div key={v.id}
                    ...
                    <div style={{ width: 22, height: 22, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", border: `2px solid ${isSel ? "var(--primary)" : "var(--border)"}`, background: isSel ? "var(--primary)" : "transparent", color: "#fff" }}>
                      {isSel && <Check className="w-3 h-3" />}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "var(--foreground)" }}>{v.label}</div>
                      <div style={{ fontSize: 12, color: "var(--muted-foreground)" }}>{v.accent} · {v.gender}</div>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); sampleVoice(v.id); }}
                      disabled={samplingVoice != null}
                      aria-label={`Play a sample of the ${v.label} voice`}
                      style={{ display: "flex", alignItems: "center", gap: 6, height: 34, padding: "0 12px", borderRadius: 9, border: "1px solid var(--border)", background: "var(--muted)", color: "var(--foreground)", fontFamily: "inherit", fontSize: 12, fontWeight: 600, cursor: samplingVoice != null ? "not-allowed" : "pointer", opacity: samplingVoice != null && samplingVoice !== v.id ? 0.5 : 1 }}>
                      {samplingVoice === v.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5" />}
                      Sample
                    </button>
                  </div>
                );
          ...
          <div>
            <button
              ...
              style={{ width: "100%", height: 52, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "var(--primary)", border: "none", borderRadius: 14, cursor: !requiredOk || isGenerating ? "not-allowed" : "pointer", color: "#fff", fontFamily: "inherit", fontSize: 15, fontWeight: 700, opacity: !requiredOk || isGenerating ? 0.5 : 1 }}
            >
            ...
            <p style={{ fontSize: 12, color: "var(--muted-foreground)", margin: "10px 0 0", lineHeight: 1.5 }}>
              You'll talk with an AI interviewer that asks one question at a time — answer by voice (mic access needed) or by typing. It stays in character like a real interview; your detailed STAR feedback and scores come at the end, and the session is saved to your history.
            </p>
          </div>
                    ...
                    <div key={s.id} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px" }}>
                        ...
                        <button onClick={() => setOpenHistoryId(open ? null : s.id)}
                          aria-label={open ? "Hide session details" : "Review session details"} aria-expanded={open}
                        ...
                        <button onClick={() => deleteSession(s.id)} title="Delete session" aria-label="Delete session"
