...
const MD_COMPONENTS = {
  ...
  p: (p: any) => (
    <p
      style={{ margin: "0 0 10px", fontSize: 13, lineHeight: 1.6, color: "var(--foreground)" }}
      {...p}
    />
  ),
  ul: (p: any) => (
  ...
  li: (p: any) => (
    <li style={{ fontSize: 13, lineHeight: 1.55, color: "var(--foreground)" }} {...p} />
  ),
...
export function JobDescription({
  ...
  useEffect(() => {
    ...
    importJobFromUrl(url)
      ...
      .catch((e) => {
        if (!cancelled)
          setError(e instanceof Error ? e.message : "Couldn't load the full description.");
      })
      .finally(() => {
  ...
  if (md) {
    return (
      <div style={{ fontSize: 13, color: "var(--foreground)" }}>
        <Markdown rehypePlugins={[rehypeSanitize]} components={MD_COMPONENTS}>
          {md}
        </Markdown>
      </div>
    );
