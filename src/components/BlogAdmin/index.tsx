...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div className="font-display text-2xl font-bold" style={{ color: "var(--foreground)" }}>
        {value}
      </div>
    </div>
  );
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground mt-1">
          ...
          <span>
            {queued
              ? `drafted ${fmtDate(post.generatedAt)}`
      ...
      {queued ? (
        <span
          className="text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0 inline-flex items-center gap-1"
          style={{ background: "rgba(232,185,72,0.15)", color: "#B7791F" }}
        >
...
export function BlogAdmin() {
  ...
  const backlog = useMemo(() => {
    ...
    return (seedTopics as { title: string; category: string }[]).filter(
      (t) => !covered.has(slugify(t.title)),
    );
  }, [queued, published]);
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto" style={{ background: "var(--background)" }}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        <header className="flex items-start justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Schedule, queued drafts, and published posts — all in one place.
            </p>
          </div>
          <button
        ...
        {/* Schedule */}
        <div
          className="rounded-2xl p-5 border mb-6 flex items-center gap-4"
          style={{ background: "var(--card)", borderColor: "var(--border)" }}
        >
          <div
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
          ...
          <StatCard
            icon={FileText}
            label="Queued (in review)"
            value={loading ? "—" : queued.length}
            tint="#E8B948"
          />
          <StatCard
            icon={CheckCircle2}
          ...
          <>
            <Section title="Scheduled · auto-publishing" count={scheduled.length}>
              <p className="text-xs text-muted-foreground mb-3 px-1">
                Posts set to publish automatically at a specific time. Switch between list and
                calendar to find when each goes live; reschedule or cancel any of them.
              </p>
              <ScheduledView
                posts={scheduled}
                onEdit={goToEditor}
            ...
            <Section title="Queued · awaiting review" count={queued.length}>
              {queued.length === 0 ? (
                ...
                <div className="space-y-2">
                  {queued.map((p) => (
                    <PostRow key={p.slug} post={p} queued onEdit={goToEditor} />
                  ))}
                </div>
              )}
            </Section>
            ...
            <Section title="Published" count={published.length}>
              {published.length === 0 ? (
                ...
                <div className="space-y-2">
                  {published.map((p) => (
                    <PostRow key={p.slug} post={p} queued={false} onEdit={goToEditor} />
                  ))}
                </div>
              )}
            </Section>
            ...
            <Section title="Upcoming topic backlog" count={backlog.length}>
              <p className="text-xs text-muted-foreground mb-3 px-1">
                Evergreen seed topics not yet covered. Click one to draft it with AI now — ahead of
                the schedule — then edit and publish it yourself. Each scheduled run also discovers
                timely trending topics via web search.
              </p>
              ...
              {genTitle && (
                <p className="text-xs text-muted-foreground mt-3 px-1">
                  Researching and drafting “{genTitle}” — this takes a moment. You'll drop into the
                  editor when it's ready.
                </p>
              )}
            </Section>
          </>
