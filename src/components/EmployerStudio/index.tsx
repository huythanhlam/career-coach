...
export function EmployerStudio(_props: Props) {
  ...
  const { listings, addListing, updateListing, deleteListing, boostListing } =
    useEmployerListings();
  ...
  const activeBoosts = useMemo(
    () => listings.filter((l) => isBoostActive(l.boostedUntil)).length,
    [listings],
  );

  // Keep the promote drawer / boost target in sync with the latest listing data.
  const liveBoostTarget = boostTarget
    ? (listings.find((l) => l.id === boostTarget.id) ?? boostTarget)
    : null;
  const livePromote = promoteListing
    ? (listings.find((l) => l.id === promoteListing.id) ?? promoteListing)
    : null;
  const companyNameOf = (l: EmployerJobListing) =>
    profiles.find((c) => c.id === l.companyId)?.name ?? "";
      ...
      >
        ...
        >
          <div>
            ...
            >
              Build your company profile, write job listings with AI, then promote and boost them to
              reach candidates.
            </p>
          </div>
        ...
        {!companiesLoading && profiles.length === 0 && (
          <div style={{ ...cardStyle, textAlign: "center", padding: 48 }}>
            ...
            >
              Set up a company profile to start posting jobs. The AI can draft your About, mission,
              culture, and benefits.
            </p>
            <button
              style={{ ...primaryBtn, margin: "0 auto" }}
              onClick={() => setMode({ kind: "company", company: null })}
            >
              <Plus className="w-4 h-4" /> New company
            </button>
          </div>
          ...
          <>
            ...
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                <h2
                  className="font-display"
                  style={{ fontSize: 20, fontWeight: 600, color: "var(--foreground)", margin: 0 }}
                >
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  ...
                  onClick={() => setMode({ kind: "company", company: selectedCompany })}
                >
            ...
            <div>
              ...
              >
                <h3
                  className="font-display"
                  style={{ fontSize: 18, fontWeight: 600, color: "var(--foreground)", margin: 0 }}
                >
                  Job listings{" "}
                ...
                <button
                  ...
                  onClick={() =>
                    setMode({ kind: "listing", listing: null, companyId: selectedCompany.id })
                  }
                >
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          padding: "14px 18px",
                          borderBottom:
                            i === companyListings.length - 1 ? "none" : "1px solid var(--border)",
                        }}
                      >
                        <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <button
                          style={{ ...ghostBtn, height: 34, padding: "0 12px" }}
                          onClick={() =>
                            setMode({ kind: "listing", listing: l, companyId: l.companyId })
                          }
                        >
                          <Pencil className="w-3.5 h-3.5" /> Edit
                        </button>
    ...
    >
      <div
        style={{
          ...
          alignItems: "center",
          justifyContent: "center",
          background: accent ? "rgba(217,119,87,0.12)" : "var(--muted)",
        }}
      >
        <Icon
          className="w-4 h-4"
      ...
      <div>
        <div style={{ fontSize: 20, fontWeight: 700, color: "var(--foreground)", lineHeight: 1 }}>
          {value}
        </div>
