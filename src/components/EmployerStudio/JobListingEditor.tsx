...
export function JobListingEditor({ listing, companyId, companyName, onSave, onCancel }: Props) {
  const [draft, setDraft] = useState<Draft>(listing ?? { companyId, title: "", status: "draft", salaryCurrency: "USD" });
  ...
  const fullContext = [draft.description, draft.requirements, draft.responsibilities].filter(Boolean).join("\n\n");
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: "var(--muted-foreground)", margin: "2px 0 0" }}>{companyName}</p>
          </div>
        </div>
        ...
        <div style={{ ...cardStyle, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 14, color: "var(--foreground)", fontWeight: 600 }}>Status</span>
              <select value={draft.status ?? "draft"} onChange={(e) => set("status", e.target.value as ListingStatus)} style={{ ...inputStyle, height: 38, width: "auto", cursor: "pointer" }}>
        ...
        {/* Generate from brief */}
        <div style={{ ...cardStyle, border: "1px solid color-mix(in srgb, var(--primary) 35%, transparent)", background: "color-mix(in srgb, var(--primary) 5%, transparent)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <Sparkles className="w-4 h-4" style={{ color: "var(--primary)" }} />
            <span style={{ fontSize: 14, fontWeight: 700, color: "var(--foreground)" }}>Generate from a brief</span>
          </div>
          <p style={{ fontSize: 13, color: "var(--muted-foreground)", margin: "0 0 12px", lineHeight: 1.5 }}>
            Jot down the key responsibilities, must-have skills, and perks. The AI drafts the description, requirements, and responsibilities below for you to review.
          </p>
          <textarea
            value={keyPoints}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12 }}>
            <button onClick={generateFromBrief} disabled={generating} style={{ ...primaryBtn, opacity: generating ? 0.7 : 1, cursor: generating ? "not-allowed" : "pointer" }}>
              {generating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}
              {generating ? "Drafting…" : "Generate description"}
            </button>
            {genError && <span style={{ fontSize: 12, color: "var(--destructive, #ef4444)" }}>{genError}</span>}
          </div>
        </div>

        {/* AI-assisted long-form fields */}
        <div style={{ ...cardStyle, display: "flex", flexDirection: "column", gap: 20 }}>
          <AITextField label="Description" value={draft.description ?? ""} onChange={(v) => set("description", v)} placeholder="Introduce the role, team, and impact…" rows={6} context={fullContext || (draft.description ?? "")} />
