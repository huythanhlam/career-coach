...
const GOAL_TYPES: GoalType[] = [
  {
    ...
    desc: "Build a new capability or specialization",
    placeholder: "e.g. Get hands-on with system design and ship one design doc per quarter",
  },
  {
    value: "Get a promotion",
    label: "Get a promotion",
    icon: <TrendingUp className="w-4 h-4" />,
    desc: "Level up in your current track",
    placeholder: "e.g. Reach Senior by demonstrating cross-team impact and leading a project",
  },
  {
    value: "Change roles",
    label: "Change roles",
    icon: <Repeat className="w-4 h-4" />,
    desc: "Move into a different role or function",
    placeholder: "e.g. Move from Backend Engineer toward Engineering Management",
  },
  {
    value: "Find a new job",
    label: "Find a new job",
    icon: <Briefcase className="w-4 h-4" />,
    desc: "Switch companies or land a new offer",
    placeholder: "e.g. Land a senior role at a product-led company by Q3",
  },
  {
    value: "Increase my compensation",
    label: "Increase compensation",
    icon: <DollarSign className="w-4 h-4" />,
    desc: "Negotiate a raise or higher total comp",
    placeholder: "e.g. Push total comp toward market rate; prepare for a raise conversation",
  },
  {
    value: "Move into leadership",
    label: "Move into leadership",
    icon: <Users className="w-4 h-4" />,
    desc: "Grow into management or tech lead",
    placeholder: "e.g. Mentor two engineers and own a workstream end-to-end",
  },
  {
    value: "Earn a certification",
    label: "Earn a certification",
    icon: <Award className="w-4 h-4" />,
    desc: "Get a credential that boosts your profile",
    placeholder: "e.g. Pass AWS Solutions Architect; study 4 hrs/week",
  },
  {
    value: "Grow my network",
    label: "Grow my network",
    icon: <Share2 className="w-4 h-4" />,
    desc: "Build relationships and visibility",
    placeholder: "e.g. Attend one event a month and publish two posts on my work",
  },
  {
    value: "Improve work-life balance",
    label: "Work-life balance",
    icon: <Scale className="w-4 h-4" />,
    desc: "Sustainable pace without stalling growth",
    placeholder: "e.g. Set boundaries while still hitting a growth milestone",
  },
  {
    value: "Transition to a new industry",
    label: "Switch industry",
    icon: <Globe className="w-4 h-4" />,
    desc: "Pivot to a new domain or sector",
    placeholder: "e.g. Move from fintech into health tech",
  },
  {
    value: OTHER,
    label: "Something else",
    icon: <Plus className="w-4 h-4" />,
    desc: "Define your own goal",
    placeholder: "Describe the goal and what you want to do this year…",
  },
];
...
export function GoalPlanIntakeForm({
  ...
  const toggle = (value: string) => setSelected((prev) => ({ ...prev, [value]: !prev[value] }));
        ...
        {selectedGoals.length > 0 && (
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 12, color: "var(--muted-foreground)", marginTop: 6 }}>
                Add specifics for each goal so the plan is concrete and measurable. Type or dictate
                with the mic, then polish with AI:
              </div>
              <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                >
                  ...
                  />
                  <span>
                    <strong style={{ color: "var(--foreground)", fontWeight: 600 }}>Refine</strong>{" "}
                    — polishes how it's written: grammar, clarity, concision, tone, and stronger
                    wording — without changing your meaning or adding ideas.
                  </span>
                </div>
                <div
                  style={{
                ...
                >
                  <Lightbulb
                    className="w-3.5 h-3.5"
                    style={{ color: "var(--marigold)", flexShrink: 0, marginTop: 2 }}
                  />
                  <span>
                    <strong style={{ color: "var(--foreground)", fontWeight: 600 }}>Suggest</strong>{" "}
                    — reviews your answer and proposes a fuller version to help complete your
                    thought.
                  </span>
                </div>
              </div>
            ...
            <div>
              <label htmlFor="goal-timeframe" style={labelStyle}>
                Target timeframe
              </label>
              <select
                id="goal-timeframe"
          ...
          >
            ...
            {submitLabel
              ...
              : selectedGoals.length > 1
                ? `Generate my plan (${selectedGoals.length} goals)`
                : "Generate my plan"}
          </button>
        </div>
        {!baselineReady && (
