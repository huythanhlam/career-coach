    ...
    <div style={{ marginTop: 10 }}>
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button style={{ ...ghostBtn, flexShrink: 0 }} disabled={busy || !url.trim()} onClick={go}>
          {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Link2 className="w-3.5 h-3.5" />}{" "}
          Import
        </button>
      </div>
    ...
    <div style={{ ...cardStyle, padding: 16 }}>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: "3 1 260px" }}>
          <Search
            className="w-4 h-4"
            style={{ position: "absolute", left: 14, top: 14, color: "var(--muted-foreground)" }}
          />
          <input
            style={{ ...inputStyle, paddingLeft: 38 }}
            placeholder="Job title or keywords (e.g. Product Manager)"
            value={keyword}
        ...
        <div style={{ position: "relative", flex: "2 1 180px" }}>
          ...
          <datalist id="loc-suggestions">
            {suggestLocations(location).map((s) => (
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
        <select
          ...
          value={level}
          onChange={(e) => onLevel(e.target.value as JobLevel | "any")}
        ...
        <select
          style={filterSelectStyle}
          value={workplace}
          onChange={(e) => onWorkplace(e.target.value as Workplace | "any")}
        >
        ...
        <select
          style={filterSelectStyle}
          value={family}
          onChange={(e) => onFamily(e.target.value as JobFamily | "any")}
        ...
        <select
          style={filterSelectStyle}
          value={industry}
          onChange={(e) => onIndustry(e.target.value as Industry | "any")}
      ...
      <div style={{ marginTop: 10 }}>
        <button
          onClick={() => onShowImport(!showImport)}
          style={{
            display: "inline-flex",
            alignItems: "center",
            gap: 6,
