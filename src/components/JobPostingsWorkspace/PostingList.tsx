...
/* ─────────────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────────────── */
const fitBarColor = (s: number) =>
  s >= 60 ? "var(--primary)" : s >= 35 ? "#F59E0B" : "var(--muted-foreground)";
    ...
    >
      <div
        style={{
          fontSize: 17,
          fontWeight: 700,
          color: recommended ? "var(--primary)" : "var(--foreground)",
          lineHeight: 1,
        }}
      >
        {score}
        ...
        createPortal(
          ...
          >
              ...
              {factors.map((f) => (
                <div key={f.key}>
                  ...
                  >
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: fitBarColor(f.score) }}>
                      {f.score}%
                    </span>
                  </div>
                  <div
                  ...
                  >
                    <div
                      style={{
                        height: "100%",
                        width: `${f.score}%`,
                        background: fitBarColor(f.score),
                        borderRadius: 9999,
                      }}
                    />
                  </div>
            ...
            <div style={{ fontSize: 10, color: "var(--muted-foreground)", marginTop: 10 }}>
              Click the row for full details
            </div>
          </div>,
          document.body,
        )}
      ...
      {onToggleFav && (
        <button
          ...
          style={{
            ...
            border: "none",
            cursor: "pointer",
            color: p?.favorite ? "var(--primary)" : "var(--muted-foreground)",
            display: "flex",
            padding: 0,
            flexShrink: 0,
          }}
    ...
    <div>
      ...
      >
        ...
        >
          ...
          {!savedOnly && personalized && recommendedCount > 0 && (
            <span
              style={{ fontSize: 13, color: "var(--primary)", fontWeight: 600, marginLeft: 10 }}
            >
      ...
      >
        {items.length === 0 ? (
          ...
          >
            {loading
              ? "Finding jobs that fit your profile…"
              : savedOnly
                ? "No saved jobs yet — search above and save the ones you like."
                : level !== "any" ||
                    workplace !== "any" ||
                    family !== "any" ||
                    industry !== "any" ||
                    location.trim()
                  ? "No jobs match these filters — try clearing the location, level, workplace, family, or industry filter."
                  : searched
                    ? "No jobs to show — try a different search."
