...
export function DocumentTitleBar({
  ...
  return (
    <div
      className="flex items-center gap-3 px-5 shrink-0 print:hidden"
    ...
    >
      ...
      <span
        ...
        onBlur={() => {
          const t = titleRef.current?.textContent?.trim() || "Untitled document";
          setTitle(t);
          onTitleChange?.(t);
          if (titleRef.current) titleRef.current.style.background = "transparent";
        }}
        onFocus={() => {
          if (titleRef.current) titleRef.current.style.background = "var(--muted)";
        }}
        onKeyDown={(e) => {
          if (e.key === "Enter") {
            e.preventDefault();
      ...
      <div style={{ fontSize: 11, color: "var(--muted-foreground)" }}>
        ...
        {saveStatus === "saved" && (
          <span style={{ display: "flex", alignItems: "center", gap: 4, color: "var(--forest)" }}>
            <Save style={{ width: 11, height: 11 }} /> Saved
          </span>
        )}
      </div>
      <div style={{ flex: 1 }} />
      <div className="flex items-center gap-2">
        <button
          ...
          title={showStylePanel ? "Hide style" : "Style"}
          style={{
            width: 32,
            height: 32,
            borderRadius: 8,
            background: showStylePanel ? "rgba(217,119,87,0.10)" : "transparent",
            border: showStylePanel ? "1px solid rgba(217,119,87,0.25)" : "1px solid var(--border)",
            cursor: "pointer",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            color: showStylePanel ? "var(--primary)" : "var(--muted-foreground)",
        ...
        {onSave && (
          <button
            onClick={() => onSave(content, title, getHtml(), docStyle)}
            style={{
              height: 32,
              padding: "0 14px",
        ...
        {aiEnabled && (
          <button
            ...
            style={{
              ...
              height: 32,
              borderRadius: 8,
              background: showAI ? "rgba(217,119,87,0.10)" : "transparent",
              border: showAI ? "1px solid rgba(217,119,87,0.25)" : "1px solid var(--border)",
              cursor: "pointer",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              color: showAI ? "var(--primary)" : "var(--muted-foreground)",
              transition: "all 0.15s",
            }}
          >
            {showAI ? (
