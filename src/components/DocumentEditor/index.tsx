...
export type { DocMessage, DocStyle, StoredDocumentPayload, DocumentEditorProps, DocumentEditorHandle } from "./types";
...
export const DocumentEditor = forwardRef<DocumentEditorHandle, DocumentEditorProps>(function DocumentEditor({
  ...
  useImperativeHandle(ref, () => ({
    ...
    setHighlight: (text) => {
      clearHighlight();
      if (editorRef.current && text) setTextHighlight(editorRef.current, text, SUGGESTION_HIGHLIGHT, clearHighlightRef);
    },
  }));
  ...
  useDocumentStyle({ scopeId: scopeId.current, rawHtmlMode, templateId: docStyle.templateId, accentColor: docStyle.accentColor, accentStyle: docStyle.accentStyle });
  ...
  const getHtml = useCallback(() => editorRef.current?.innerHTML ?? markdownToHtml(content), [content]);
  const exportPDF = useCallback(() => exportToPDF({ title, docStyle, html: getHtml(), content }), [title, docStyle, getHtml, content]);
  const exportDocx = useCallback(() => exportToDocx({ docStyle, html: getHtml(), content, exportFileName, title, headerHtml }), [docStyle, getHtml, content, exportFileName, title, headerHtml]);
  ...
  return (
    <div className="flex flex-col h-full w-full" style={{ background: "var(--background)", fontFamily: "var(--font-sans, Inter, sans-serif)" }}>
      ...
      {/* Title bar */}
      <DocumentTitleBar
        title={title} setTitle={setTitle} onTitleChange={onTitleChange}
        saveStatus={saveStatus}
        showStylePanel={showStylePanel} setShowStylePanel={setShowStylePanel}
        showAI={showAI} setShowAI={setShowAI}
        aiEnabled={aiEnabled} onSave={onSave} onClose={onClose}
        content={content} docStyle={docStyle}
        getHtml={getHtml}
