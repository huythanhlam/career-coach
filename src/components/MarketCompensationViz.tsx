...
import { DollarSign, TrendingUp, Building2, ExternalLink, MapPin, Calculator, RefreshCw, Trophy, ShieldCheck, Info, Target, Layers, Receipt } from 'lucide-react';
...
const makeFmt = (currency = "USD") => (val: number) =>
...
const totalCol = (c: LocationCompData["costOfLiving"]) =>
  (c.housing || 0) + (c.utilities || 0) + (c.gas || 0) + (c.groceries || 0) + (c.dining || 0) + (c.transportation || 0) + (c.healthcare || 0);
...
const cardStyle: React.CSSProperties = { background: "var(--card)", border: "1px solid var(--border)", borderRadius: 24, boxShadow: "0 4px 20px rgba(0,0,0,0.04)" };
...
function SourceBadge({ source, prefix = "Source" }: { source?: SourceRef; prefix?: string }) {
  ...
  return (
    ...
    >
      ...
      <span>{prefix}: {source.label}{source.asOf ? ` · ${source.asOf}` : ""}</span>
    </a>
  );
}
...
function ConfidenceChip({ level }: { level?: "high" | "medium" | "low" }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full" style={{ background: s.bg, color: s.color }}>
      <ShieldCheck className="w-3 h-3" /> {s.label}
    </span>
  );
}

export function MarketCompensationViz({ data, cachedAt, onRefresh, isRefreshing, onNavigate }: MarketCompensationVizProps) {
    ...
    <div className="w-full space-y-6 mt-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 font-semibold" style={fgText}>
          ...
          <span className="text-lg">{data.locations.map(l => l.locationName).join("  vs  ")}</span>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {data.locations.map((l, i) => l.confidence && (
            <ConfidenceChip key={i} level={l.confidence} />
          ))}
          {cachedAt && (
          ...
          {onRefresh && (
            <button
              ...
              style={{ background: "var(--card)", border: "1px solid var(--border)", color: "var(--foreground)", cursor: isRefreshing ? "default" : "pointer" }}
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isRefreshing ? "animate-spin" : ""}`} /> {isRefreshing ? "Refreshing…" : "Refresh data"}
            </button>
          )}
        </div>
      </div>
      ...
      {isComparison ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {data.locations.map((loc, i) => {
            ...
            return (
              <Card key={i} style={{ ...cardStyle, background: `${accent}0F`, border: `1px solid ${accent}33` }}>
                <CardHeader className="pb-2">
                  <CardTitle className="text-xl" style={fgText}>{loc.locationName}</CardTitle>
                ...
                <CardContent className="p-6 pt-2">
                  <div className="grid grid-cols-2 gap-5">
                    <div>
                      ...
                      <div className="mt-1"><SourceBadge source={loc.salaryBands.source} /></div>
                    </div>
                    <div>
                      <p className="text-xs font-medium mb-1 flex items-center gap-1" style={muteText}><Calculator className="w-3 h-3" /> {hasTax(loc) ? "Take-home (after tax & COL)" : "Real Income (after COL)"}</p>
                      <h3 className="text-3xl font-bold leading-tight" style={{ color: accent }}>{lfmt(takeHome(loc))}</h3>
                      {hasTax(loc) && <p className="text-xs mt-0.5" style={muteText}>~{Math.round((loc.effectiveTaxRate as number) * 100)}% effective tax</p>}
                      <div className="mt-1"><SourceBadge source={loc.costOfLiving.source} /></div>
                    </div>
                  </div>
                  {loc.totalCompensation && (
                    <div className="pt-4 mt-4" style={{ borderTop: "1px solid var(--border)" }}>
                      ...
                      <div className="grid grid-cols-4 gap-2 text-center text-sm">
                        ...
                        <div><div className="font-bold" style={{ color: accent }}>{lfmt(loc.totalCompensation.totalEstimated)}</div><div className="text-xs" style={muteText}>Total</div></div>
                      </div>
                      <div className="mt-2"><SourceBadge source={loc.totalCompensation.source} /></div>
                    </div>
                  )}
                </CardContent>
              </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <HeroStat
            ...
            sub={hasTax(primaryLoc) ? `~${Math.round((primaryLoc.effectiveTaxRate as number) * 100)}% effective tax` : undefined}
            accent="#2F6B4F"
          ...
          {primaryLoc.totalCompensation ? (
            <Card style={cardStyle}>
              <CardContent className="p-6">
                <p className="text-sm font-medium mb-3" style={fgText}>Total Compensation (TC)</p>
                <div className="flex justify-between items-center p-2.5 rounded-lg mb-3" style={{ background: "var(--muted)", border: "1px solid var(--border)" }}>
                  <span className="text-xs font-medium" style={muteText}>Estimated TC</span>
                  <span className="text-lg font-bold" style={{ color: "#D97757" }}>{fmt(primaryLoc.totalCompensation.totalEstimated)}</span>
                </div>
                {[
                  ...
                  <div key={label as string} className="flex justify-between items-center text-sm mb-1.5" style={muteText}>
                    <span>{label}</span>
                    <span className="font-medium" style={fgText}>{fmt(val as number)}</span>
                  </div>
                ))}
                <div className="mt-2"><SourceBadge source={primaryLoc.totalCompensation.source} /></div>
              </CardContent>
            </Card>
          ) : (
            <Card style={cardStyle}>
              <CardContent className="p-6 flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm leading-snug pr-2" style={muteText}>{primaryLoc.equity}</p>
                </div>
                <div style={{ width: 44, height: 44, borderRadius: "50%", background: "var(--muted)", display: "flex", alignItems: "center", justifyContent: "center", ...muteText, flexShrink: 0 }}>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <Card style={{ ...cardStyle, overflow: "hidden" }}>
          ...
          <div className="px-6 pb-4"><SourceBadge source={primaryLoc.costOfLiving.source} /></div>
        </Card>
      </div>
      ...
      <Card style={{ ...cardStyle, overflow: "hidden" }}>
        ...
        <div className="px-6 pb-4"><SourceBadge source={primaryLoc.yoyTrendSource} /></div>
      </Card>

      {/* References + disclaimer */}
      {data.sources && data.sources.length > 0 && (
        <div className="flex flex-wrap gap-2 text-sm items-center" style={muteText}>
          <span className="font-medium mr-1.5 flex items-center"><ExternalLink className="w-3.5 h-3.5 mr-1" /> All references:</span>
          {data.sources.map((s, idx) => {
            ...
            return (
              <a key={idx} href={href} target="_blank" rel="noopener noreferrer" style={{ color: "var(--primary)" }} className="hover:underline">
                {host}
              </a>
            );
          })}
      ...
      <p className="text-xs flex items-start gap-1.5" style={muteText}>
        ...
        Estimates aggregated from public sources — not a guarantee of pay. Verify against the linked sources before negotiating.
      </p>
    </div>
  );
}
...
function VerdictHero({ data, fmt }: { data: MarketCompData; fmt: (v: number) => string }) {
  ...
  return (
    <div style={{ ...cardStyle, padding: 24, background: "linear-gradient(135deg, rgba(217,119,87,0.08), rgba(47,107,79,0.08))" }}>
      <div className="flex items-start gap-3">
        <div style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(217,119,87,0.15)", display: "flex", alignItems: "center", justifyContent: "center", color: "#D97757", flexShrink: 0 }}>
          <Trophy className="h-5 w-5" />
        </div>
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider mb-1" style={muteText}>The verdict — real pay after {afterPhrase}</p>
          <p className="text-xl font-bold leading-snug" style={fgText}>
            {realGap === 0
              ? <>Both locations leave you with roughly the same take-home.</>
              : <>After {afterPhrase}, <span style={{ color: "#2F6B4F" }}>{realWinner.locationName}</span> leaves you <span style={{ color: "#2F6B4F" }}>+{fmt(realGap)}/yr</span> more than {realLoser.locationName}.</>}
          </p>
          <p className="text-sm mt-1.5" style={muteText}>
            Nominal median base is higher in <strong style={fgText}>{nominalWinner}</strong> ({fmt(Math.abs(nominalDelta))} gap).
...
function ComparisonTable({ data, fmt }: { data: MarketCompData; fmt: (v: number) => string }) {
  const [a, b] = data.locations;
  const taxAware = hasTax(a) && hasTax(b);
  const rows: { label: string; va: number; vb: number; better: "high" | "low"; highlight?: boolean }[] = [
    { label: "Median base salary", va: a.salaryBands.median, vb: b.salaryBands.median, better: "high" },
    { label: "Total compensation", va: a.totalCompensation?.totalEstimated ?? a.salaryBands.median, vb: b.totalCompensation?.totalEstimated ?? b.salaryBands.median, better: "high" },
    { label: "Total monthly cost of living", va: totalCol(a.costOfLiving), vb: totalCol(b.costOfLiving), better: "low" },
    { label: taxAware ? "Take-home (after tax & COL)" : "Real income (after COL)", va: takeHome(a), vb: takeHome(b), better: "high", highlight: true },
  ];
  ...
  return (
    <Card style={{ ...cardStyle, overflow: "hidden" }}>
      <CardHeader className="p-6 pb-2">
        <CardTitle className="text-base" style={fgText}>Side-by-side comparison</CardTitle>
        <CardDescription style={muteText}>Winner highlighted per metric · Δ is {b.locationName} minus {a.locationName}</CardDescription>
      </CardHeader>
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="w-full text-sm text-left">
            <thead style={{ borderBottom: "1px solid var(--border)" }}>
              <tr>
                <th className="px-3 py-2.5 font-medium" style={muteText}>Metric</th>
                <th className="px-3 py-2.5 font-semibold text-right" style={{ color: PALETTE[0] }}>{a.locationName}</th>
                <th className="px-3 py-2.5 font-semibold text-right" style={{ color: PALETTE[1] }}>{b.locationName}</th>
                <th className="px-3 py-2.5 font-medium text-right" style={muteText}>Δ</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.label} style={{ borderBottom: "1px solid var(--border)", background: r.highlight ? "var(--muted)" : undefined }}>
                    <td className="px-3 py-3 font-medium" style={fgText}>{r.label}</td>
                    <td className="px-3 py-3 text-right font-semibold" style={{ color: aWins ? winnerColor : "var(--foreground)" }}>
                      {fmt(r.va)}{aWins && " ✓"}
                    </td>
                    <td className="px-3 py-3 text-right font-semibold" style={{ color: bWins ? winnerColor : "var(--foreground)" }}>
                      {fmt(r.vb)}{bWins && " ✓"}
                    </td>
                    <td className="px-3 py-3 text-right" style={{ color: delta === 0 ? "var(--muted-foreground)" : delta > 0 ? "#2F6B4F" : "#D97757" }}>
                      {delta > 0 ? "+" : ""}{fmt(delta)}
                    </td>
                  </tr>
                );
...
function HeroStat({ label, value, sub, accent, icon, source }: { label: string; value: string; sub?: string; accent: string; icon: React.ReactNode; source?: SourceRef }) {
  return (
    <Card style={{ ...cardStyle, background: `${accent}0F`, border: `1px solid ${accent}33` }}>
      <CardContent className="p-6">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-sm font-medium mb-1" style={{ color: accent, opacity: 0.85 }}>{label}</p>
            <h3 className="text-4xl font-bold leading-tight" style={{ color: accent }}>{value}</h3>
            {sub && <p className="text-xs mt-1" style={muteText}>{sub}</p>}
          </div>
          <div style={{ width: 44, height: 44, borderRadius: "50%", background: `${accent}1F`, display: "flex", alignItems: "center", justifyContent: "center", color: accent, flexShrink: 0 }}>
            {icon}
          </div>
        </div>
        <div className="mt-3"><SourceBadge source={source} /></div>
      </CardContent>
    </Card>
  );
}
...
function percentileOf(v: number, b: LocationCompData["salaryBands"]): number {
  const pts: [number, number][] = [[b.min, 0], [b.q1, 25], [b.median, 50], [b.q3, 75], [b.max, 100]];
  if (v <= b.min) return 0;
...
function OfferGauge({ loc, fmt, onNavigate }: { loc: LocationCompData; fmt: (v: number) => string; onNavigate?: (view: string) => void }) {
  ...
  const markerPos = valid ? Math.min(100, Math.max(0, ((offer - b.min) / ((b.max - b.min) || 1)) * 100)) : null;

  let verdict = "";
  let verdictColor = "var(--foreground)";
  if (pct !== null) {
    if (pct < 25) { verdict = "below the 25th percentile — strong room to negotiate up."; verdictColor = "#D97757"; }
    else if (pct < 45) { verdict = "below the median — likely room to push higher."; verdictColor = "#D97757"; }
    else if (pct <= 55) { verdict = "right around the median for this market."; }
    else if (pct <= 75) { verdict = "above the median — a competitive offer."; verdictColor = "#2F6B4F"; }
    else { verdict = "above the 75th percentile — a strong offer."; verdictColor = "#2F6B4F"; }
  }

  return (
    <Card style={{ ...cardStyle, overflow: "hidden" }}>
      <CardHeader className="p-6 pb-2">
        <CardTitle className="text-base flex items-center gap-2" style={fgText}><Target className="w-4 h-4" /> Where does your offer land?</CardTitle>
        <CardDescription style={muteText}>Compare your current or offered base against {loc.locationName} ({fmt(b.min)}–{fmt(b.max)})</CardDescription>
      </CardHeader>
      <CardContent className="p-6 pt-3">
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <input
            ...
            style={{ background: "var(--muted)", border: "1px solid var(--border)", color: "var(--foreground)", outline: "none", width: 180 }}
          />
          {pct !== null && (
        ...
        <div style={{ position: "relative", height: 10, borderRadius: 9999, background: "linear-gradient(90deg, rgba(217,119,87,0.35), rgba(232,185,72,0.35), rgba(47,107,79,0.35))" }}>
          {([["25th", b.q1], ["Median", b.median], ["75th", b.q3]] as [string, number][]).map(([lbl, val]) => {
            const pos = Math.min(100, Math.max(0, ((val - b.min) / ((b.max - b.min) || 1)) * 100));
            return <div key={lbl} style={{ position: "absolute", left: `${pos}%`, top: -3, width: 1, height: 16, background: "var(--border)" }} />;
          })}
          {markerPos !== null && (
            <div style={{ position: "absolute", left: `${markerPos}%`, top: -5, transform: "translateX(-50%)", width: 4, height: 20, borderRadius: 2, background: verdictColor }} />
          )}
        </div>
        <div className="flex justify-between mt-2 text-xs" style={muteText}>
          <span>{fmt(b.min)}</span><span>Median {fmt(b.median)}</span><span>{fmt(b.max)}</span>
        </div>
...
function LevelLadder({ data, fmt }: { data: MarketCompData; fmt: (v: number) => string }) {
  ...
  const levels = Array.from(new Set(withLadder.flatMap(l => (l.levelLadder || []).map(x => x.level))));
  ...
  return (
    <Card style={{ ...cardStyle, overflow: "hidden" }}>
      <CardHeader className="p-6 pb-2">
        <CardTitle className="text-base flex items-center gap-2" style={fgText}><Layers className="w-4 h-4" /> Compensation by level</CardTitle>
        <CardDescription style={muteText}>Typical median base as you progress in seniority</CardDescription>
      </CardHeader>
      <CardContent className="h-[300px] pt-4">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
            ...
            <YAxis tickFormatter={(val) => `$${val / 1000}k`} axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#6E6557' }} width={60} />
            <RechartsTooltip formatter={(val: number) => [fmt(val), 'Base median']} contentStyle={{ borderRadius: 8, border: '1px solid var(--border)', background: 'var(--card)', color: 'var(--foreground)', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} cursor={{ fill: 'rgba(0,0,0,0.04)' }} />
            {isComparison && <Legend wrapperStyle={{ fontSize: '12px' }} />}
            {withLadder.map((l, i) => (
              <Bar key={l.locationName} dataKey={l.locationName} fill={PALETTE[i % PALETTE.length]} radius={[4, 4, 0, 0]} maxBarSize={48} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </CardContent>
...
function SalaryRangeBar({ data, fmt }: { data: MarketCompData; fmt: (v: number) => string }) {
  ...
  const edge = (p: number): React.CSSProperties =>
    p < 8 ? { left: `${p}%`, transform: "translateX(0)" }
    : p > 92 ? { left: `${p}%`, transform: "translateX(-100%)" }
    : { left: `${p}%`, transform: "translateX(-50%)" };

  return (
    <Card style={{ ...cardStyle, overflow: "hidden" }}>
      <CardHeader className="p-6 pb-2">
        <CardTitle className="text-base" style={fgText}>Salary Range</CardTitle>
        <CardDescription style={muteText}>Box = middle 50% (25th–75th percentile); line = median; whiskers = min–max base</CardDescription>
      </CardHeader>
      <CardContent className="p-6 pt-5" style={{ display: "flex", flexDirection: "column", gap: 26 }}>
        {locs.map((loc, i) => {
          const b = loc.salaryBands;
          const accent = PALETTE[i % PALETTE.length];
          const lfmt = makeFmt(loc.currency);
          return (
            <div key={i}>
              <div className="mb-2">
                <span className="text-sm font-semibold" style={{ color: accent }}>{loc.locationName}</span>
              </div>
              {/* median value, positioned above its marker */}
              <div style={{ position: "relative", height: 16 }}>
                <span className="text-xs font-semibold" style={{ position: "absolute", whiteSpace: "nowrap", color: accent, ...edge(pos(b.median)) }}>
                  Median {lfmt(b.median)}
                </span>
              </div>
              <div style={{ position: "relative", height: 30 }}>
                {/* whisker */}
                <div style={{ position: "absolute", top: 14, left: `${pos(b.min)}%`, width: `${pos(b.max) - pos(b.min)}%`, height: 2, background: `${accent}66` }} />
                {/* min/max end caps */}
                {[b.min, b.max].map((v, k) => (
                  <div key={k} style={{ position: "absolute", top: 9, left: `${pos(v)}%`, transform: "translateX(-50%)", width: 2, height: 12, background: `${accent}99` }} />
                ))}
                {/* IQR box */}
                <div style={{ position: "absolute", top: 5, left: `${pos(b.q1)}%`, width: `${pos(b.q3) - pos(b.q1)}%`, height: 20, background: `${accent}33`, border: `1px solid ${accent}`, borderRadius: 6 }} />
                {/* median */}
                <div style={{ position: "absolute", top: 1, left: `${pos(b.median)}%`, transform: "translateX(-50%)", width: 3, height: 28, background: accent, borderRadius: 2 }} title={`Median ${lfmt(b.median)}`} />
              </div>
              <div style={{ position: "relative", height: 16, marginTop: 2 }}>
                <span className="text-xs" style={{ position: "absolute", ...edge(pos(b.min)), ...muteText }}>{lfmt(b.min)}</span>
                <span className="text-xs" style={{ position: "absolute", ...edge(pos(b.max)), ...muteText }}>{lfmt(b.max)}</span>
              </div>
            </div>
          );
        })}
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs pt-1" style={muteText}>
          <span className="inline-flex items-center gap-1.5"><span style={{ width: 16, height: 11, background: `${PALETTE[0]}33`, border: `1px solid ${PALETTE[0]}`, borderRadius: 3, display: "inline-block" }} /> Middle 50% (25th–75th)</span>
          <span className="inline-flex items-center gap-1.5"><span style={{ width: 3, height: 13, background: PALETTE[0], borderRadius: 2, display: "inline-block" }} /> Median</span>
          <span>Whiskers: min – max</span>
        </div>
      </CardContent>
      <div className="px-6 pb-4"><SourceBadge source={locs[0].salaryBands.source} /></div>
    </Card>
  );
}
...
export function marketToMarkdown(data: MarketCompData): string {
  ...
  data.locations.forEach(loc => {
    ...
    lines.push(`- Median base: ${f(loc.salaryBands.median)} (range ${f(loc.salaryBands.min)}–${f(loc.salaryBands.max)})`);
    if (loc.totalCompensation) lines.push(`- Total comp (median): ${f(loc.totalCompensation.totalEstimated)}`);
    lines.push(`- Total monthly cost of living: ${f(totalCol(loc.costOfLiving))}`);
    lines.push(`- ${hasTax(loc) ? "Take-home after tax & COL" : "Real income after COL"}: ${f(takeHome(loc))}`);
    if (loc.dataAsOf) lines.push(`- Data as of: ${loc.dataAsOf}${loc.confidence ? ` (${loc.confidence} confidence)` : ""}`);
  });
