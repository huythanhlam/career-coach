...
export type ResumeFormAction =
  ...
  | { type: "SET_STEP0_RESUME_UPLOADED"; payload: { text: string; fileName: string; file: File } | null }
...
export function makeInitialState(profile: {
  ...
  workHistory?: Array<{ company: string; role: string; startDate: string; endDate: string; responsibilities: string; current?: boolean }> | null;
  education?: Array<{ university: string; degree: string; graduationYear: string; major?: string | null; minor?: string | null }> | null;
  skills?: string[] | null;
}): ResumeFormState {
  return {
    step: 0,
...
export function resumeFormReducer(state: ResumeFormState, action: ResumeFormAction): ResumeFormState {
  switch (action.type) {
    ...
    case "SET_PERSONAL_INFO": return { ...state, personalInfo: { ...state.personalInfo, ...action.payload } };
