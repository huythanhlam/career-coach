...
function MiniTemplatePreview({ type }: { type: string }) {
  if (type === "Modern & Clean") {
    return (
      <div
        className="w-full h-full bg-white shadow-sm p-2.5 flex flex-col gap-2 rounded-sm overflow-hidden"
      ...
      >
        ...
        <div className="flex gap-2.5 h-full pt-1">
          <div
            className="w-1/3 flex flex-col gap-2 pr-2"
          ...
          >
            ...
            <div
              className="h-1.5 w-full rounded-sm mt-1"
              style={{ background: "rgba(217,119,87,0.18)" }}
            ></div>
            <div className="space-y-1">
              <div className="h-1 w-full rounded-px" style={{ background: "var(--border)" }}></div>
          ...
          <div className="w-2/3 flex flex-col gap-2">
            ...
            <div className="space-y-1 pb-1">
              <div className="flex justify-between">
                <div
                  className="h-1.5 w-1/2 rounded-px"
      ...
      <div className="w-full h-full bg-[#0d1117] border border-zinc-800 p-2.5 flex flex-col gap-1.5 rounded-sm overflow-hidden font-mono">
        ...
        <div className="space-y-1 mt-1">
          <div className="flex gap-2 items-center">
            ...
            <div className="h-1.5 w-1/3 bg-zinc-300 rounded-sm"></div>
          </div>
  ...
  if (type === "Creative / Portfolio") {
    return (
      <div
        className="w-full h-full bg-[#fdfbf7] p-2 flex flex-col gap-2 rounded-sm overflow-hidden"
        style={{ border: "1px solid rgba(217,119,87,0.25)" }}
      >
        <div className="flex items-center gap-2">
          <div
            className="w-7 h-7 rounded-full shrink-0 shadow-sm"
            style={{ background: "linear-gradient(135deg, #E8B948, #D97757)" }}
          ></div>
          <div className="flex flex-col gap-1 w-full">
            <div className="h-2 w-2/3 rounded-sm" style={{ background: "var(--foreground)" }}></div>
            <div
              className="h-1.5 w-1/3 rounded-sm"
              style={{ background: "rgba(217,119,87,0.6)" }}
            ></div>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 mt-1">
          <div
            className="h-10 rounded-sm p-1 flex items-end"
  ...
  return (
    <div
      className="w-full h-full bg-white p-2.5 flex flex-col gap-1.5 rounded-sm overflow-hidden"
      style={{ border: "1px solid var(--border)" }}
    >
      <div
        className="h-2.5 w-2/5 mb-1.5 rounded-sm"
        style={{ background: "var(--foreground)" }}
      ></div>
      {[
        ["3/4", "full", "5/6"],
        ...
        <div key={i} className="flex gap-2.5 mb-1.5">
          <div
            className="w-0.5 h-full ml-1 rounded-full"
            style={{ background: "var(--border)" }}
          ></div>
          <div className="flex flex-col gap-1.5 w-full -ml-1">
            <div
              className={`h-1.5 w-${lines[0]} rounded-px`}
              style={{ background: "var(--muted-foreground)", opacity: 0.5 }}
            ></div>
            {lines.slice(1).map((w, j) => (
              <div
                key={j}
                className={`h-1 w-${w} rounded-px`}
                style={{ background: "var(--border)" }}
              ></div>
            ))}
          </div>
...
const TEMPLATES = [
  {
    id: "Modern & Clean",
    name: "Modern & Clean",
    description: "Minimalist and professional layout.",
  },
  {
    id: "Tech Focused",
    name: "Tech Focused",
    description: "Highlight skills and projects for IT roles.",
  },
  { id: "Executive", name: "Executive", description: "Traditional, authoritative structure." },
  {
    id: "Creative / Portfolio",
    name: "Creative / Portfolio",
    description: "Vibrant visual identity for design roles.",
  },
  {
    id: "Photography / Visual",
    name: "Photography / Visual",
    description: "Grid layout for prioritizing image portfolios.",
  },
  {
    id: "Academic / Research",
    name: "Academic / Research",
    description: "Detailed format for publications and studies.",
  },
];
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {allTemplates.map((t) => (
        ...
        >
          ...
          >
            {t.id === "Match uploaded style" ? (
              ...
              >
                ...
                <span
                  style={{ fontSize: 11, color: "var(--muted-foreground)", textAlign: "center" }}
                >
                  Mirror uploaded PDF layout
                </span>
              </div>
            ) : (
          ...
          >
            <div
              style={{ fontSize: 13, fontWeight: 600, color: "var(--foreground)", marginBottom: 2 }}
            >
              {t.name}
            </div>
