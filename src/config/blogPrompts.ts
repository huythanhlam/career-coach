/**
 * System prompts for the editorial agents. They share a base voice adapted from
 * the app's coaching persona (professional, actionable, honest, cites real
 * sources) so blog content reads consistently with the rest of TechCoach AI.
 *
 * These live in src/config so both the frontend (in-app admin authoring, via the
 * AI gateway) and the CI pipeline (scripts/blog, which re-exports from here) draw
 * from one source. Pure string constants only — safe to import in either runtime.
 */
